...
import mtable2.io.MProperties;
import mtable2.model.MTable;
import mtable2.tcp.v1.Status;
import mtable2.tcp.v1.TcpClient;
import mtable2.tcp.v1.TcpServer;
...
public class TcpClientJDialog extends javax.swing.JDialog {
     ...
     */
    public TcpClientJDialog(java.awt.Frame parent, boolean modal,  IToolsDialog callBack) {
        super(parent, modal);
         this.callback = callBack;
        initComponents();
        JtxtIp.setText("");
        trySetIpFromProperties();
        jTxtPort.setText("");
    ...
    private void runClientSender() {
        ...
        List<MTable> mtables;
        try {
            mtables = callback.getDataControlInterface().getAllTables();
        } catch (Exception ex) {
            Logger.getLogger(TcpClientJDialog.class.getName()).log(Level.SEVERE, null, ex);
             jTxtStatus.setText("ERROR: "+ex.getMessage());
            resetClientInfo();
                ...
                */
                ...
                if (tcpClient.getStatus() == Status.ERROR) {
                 ...
                 jTxtStatus.setText(jTxtStatus.getText()+"\nReceiving is Completed!");
                    ...
                    mtables = tcpClient.getMTables(); //null in case of error
                
                //Important! See comment above
                if(tcpClient.getStatus()==Status.COMPLETED){
                    try {
                       callback.getDataControlInterface().saveAllTables(mtables);
                       callback.tcpReceivingCompletedUpdateListView();
                    } catch (Exception ex) {
                        Logger.getLogger(TcpClientJDialog.class.getName()).log(Level.SEVERE, null, ex);
                         jTxtStatus.setText(jTxtStatus.getText()+"\n"+ex.getMessage());
                    }
    ...
    private void trySetIpFromProperties() {
        try {
    ...
    private void saveIpInProperties() {
        String sip = JtxtIp.getText();
     if(sip!=null && !sip.isEmpty()){
            try {
                MProperties.create().saveIpProperty(sip);
            } catch (Exception ex) {
                Logger.getLogger(TcpClientJDialog.class.getName()).log(Level.SEVERE, null, ex);
            }
      }
